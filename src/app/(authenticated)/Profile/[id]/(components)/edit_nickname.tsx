...
export default  function Edit_nickname( props:any ) {
  // if (!props)
  //   return ;
    const [NickName, setNickName] = useState(props.user_data.nickname);
  ...
  const handleInputChange = (event:any) => { // here i keep the state of the updated NickName updated when ever a character is wroten or removed from the input field
    ...
    const updateNickname = () => { // here i check the values that will be POSTED to the back-end for any error
      ...
      if (updatenick.length > 32 || updatenick.length == 0 || !validInput.test(updatenick))
      {
        if (nickname_state)
    ...
    return (  
        <div className="h-full w-full edit_n ickname bg-primary_blue flex flex-col items-center space-y-10 pt-10">
            ...
            <div className="h-fit w-fit border border-pink-500 relative flex justify-center ">
                ...
                <button  className=" w-9 h-6 bg-red-500 font-bold text-white absolute bottom-[-10px]" onClick={handleClickProgrammatically} > Edit </button>
                ...
                {errimg ? 
             <label  className="font-bold pt-2 border-solid text-xs  text-red-600 "> Wrong Image </label>
             : null}
            </div>
            <label  className="font-bold text-white"> {NickName} </label>
            <div className="h-fit w-fit flex flex-col items-center">
            <input type="text" id='inpt_nickname' className="outline-none bg-primary_blue border-b-2 border-white-500  text-sm font-bold text-white" placeholder="NickName" onChange={handleInputChange} onKeyDown={handleInputChangek} />
             {errNickName ? 
             <label  className="font-bold pt-0 border-solid text-xs text-red-600 "> Wrong NickName </label>
             : null}
            </div>
            <button className="bg-red-500 h-10 w-16 font-bold text-white" onClick={updateNickname}> Save </button>
        </div>
    );
}